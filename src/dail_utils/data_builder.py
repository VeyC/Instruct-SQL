...
from dail_utils.utils import get_tables, sql2skeleton
from dail_utils.linking_utils.application import get_question_pattern_with_schema_linking
...
class BasicDataset(object):
    ...
    def get_test_json(self, mini_set=False):
        print("[DEBUG] get_test_json from:", self.test_json)
        tests = json.load(open(self.test_json, "r"))
        print("[DEBUG] Number of loaded test samples:", len(tests))
        print("[DEBUG] Example test sample:", tests[0])
        if mini_set and self.mini_test_index_json:
            mini_test_index = self.get_mini_index()
    ...
    def get_test_schema_linking(self, mini_set=False):
        print("[DEBUG] get_test_schema_linking from:", self.path_test_schema_linking)
...
class BirdDataset(BasicDataset):
    # 不要直接写死文件名
    def __init__(self, path_data, pre_test_result=None, mode="dev"):
        self.name = "bird"
        self.test_json = "dev.json" if mode == "dev" else "test.json"
        self.test_gold = "dev.sql" if mode == "dev" else "test.sql"
        self.train_json = "train.json"
        self.train_gold = "train_gold.sql"
        self.table_json = "tables.json"
        self.mini_test_index_json = None
        # 关键：拼接路径用 join 保证兼容
        enc_dir = os.path.join(path_data, self.name, "enc")
        if mode == "dev":
            self.path_test_schema_linking = os.path.join(enc_dir, "dev_schema-linking.jsonl")
        else:
            self.path_test_schema_linking = os.path.join(enc_dir, "test_schema-linking.jsonl")
        # 保证 train linking 也能用
        self.path_train_schema_linking = os.path.join(enc_dir, "train_schema-linking.jsonl")
        super().__init__(path_data, pre_test_result)


def load_data(data_type, path_data, pre_test_result=None, mode="dev"):
    if data_type.lower() == "spider":
        return SpiderDataset(path_data, pre_test_result, mode=mode)
    elif data_type.lower() == "realistic":
        return RealisticDataset(path_data, pre_test_result, mode=mode)
    elif data_type.lower() == "bird":
        return BirdDataset(path_data, pre_test_result, mode=mode)
    else:
        raise RuntimeError()
