...
import sys
import json
from dail_prompt.prompt_builder import prompt_factory
from dail_utils.data_builder import load_data
from dail_utils.enums import REPR_TYPE, EXAMPLE_TYPE, SELECTOR_TYPE, LLM
from dail_utils.utils import cost_estimate
...
sys.path.append("./")
def extract_examples_from_questions(questions_path, mode):
    # 读取 questions.json
    ...
    for q in data["questions"]:
        prompt = q["prompt"]
        split_token = "/* Given the following database schema:"
    ...
    # bird_{mode}.json 输出到 questions.json 同目录
    out_path = os.path.join(os.path.dirname(questions_path), f"bird_{mode}.json")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_ans_len", type=int, default=4096, help="The maximal length that an answer takes")
    ...
    data = load_data(args.data_type, PATH_DATA, args.pre_test_result, mode=args.mode)
    print("[INFO] Dataset test_json:", data.test_json)
    print("[INFO] Dataset path_test_schema_linking:", data.path_test_schema_linking)
    ...
    path_generate = os.path.join("..", "output", "bird", f"{args.mode}")
        
    os.makedirs(path_generate, exist_ok=True)
    json.dump(task, open(os.path.join(path_generate, "questions.json"), "w"), indent=4)
    # 生成 bird_{mode}.json 文件
    questions_path = os.path.join(path_generate, "questions.json")
    extract_examples_from_questions(questions_path, args.mode)
