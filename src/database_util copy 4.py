...
def extract_columns_from_schema(schema_text):
    """
    提取schema中的所有字段名（经过format_table_column_name，忽略主键/约束/表名等）
    """
    cols = []
    ...
    for line in lines:
        ...
        if m:
            col = m.group(1)
            if col in ["PRIMARY", "TABLE", "CONSTRAINT"]:
                continue
...
def fallback_fix_table_columns(single_table_schema_text, table_with_comments_reponse):
    ...
    for idx, line in enumerate(llm_lines):
        ...
        if re.match(r'^\s*CREATE\s+TABLE', line, re.IGNORECASE):
            ...
            inside_col_block = True
            continue
        # 字段区的左括号行(允许在CREATE TABLE行合并了(也合规)
        if inside_col_block and '(' in line:
            fixed_lines.append(line)
            # 插在这里
            if not first_insert_done and missing_cols:
                ...
                for sline in schema_lines:
                    col_match = re.match(r'\s*(`[^`]+`|\w+)\s+\w+', sline)
                    if col_match:
                        ...
                        if scol_raw in missing_cols:
                            ...
                            fixed_lines.append(f"{type_anno} -- format:")
        ...
        for sline in schema_lines:
            ...
            if col_match:
                ...
                if scol_raw in missing_cols:
                    ...
                    code_part = code_part.rstrip(',')
                    type_anno = code_part if code_part.endswith(',') else code_part + ','
                    insert_lines.append(f"{type_anno} -- format:")
