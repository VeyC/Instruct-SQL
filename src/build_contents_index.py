...
# This code is from Snowflake Inc.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", required=True, help="Data partition: dev, train, or test")
    ...
    if os.path.exists(args.index_root):
        print('索引已存在，跳过！')
        ...
        db_dir = os.path.join(args.db_root, args.mode, f"{args.mode}_databases")
        for db_id in os.listdir(db_dir):
            db_subdir = os.path.join(db_dir, db_id)
            db_path = os.path.join(db_subdir, f"{db_id}.sqlite")
            idx_path = os.path.join(args.index_root, db_id)
            if not os.path.isfile(db_path):
                print(f"Skipping missing or non-file {db_path}")
                continue
            if os.path.isdir(idx_path) and os.listdir(idx_path):
                print(f'索引 {idx_path} 已存在，跳过！')
                continue
            print(f"Processing {db_path} -> {idx_path}")
            build_content_index(db_path, idx_path, args.temp_dir, args.threads)
