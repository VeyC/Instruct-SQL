...
# This coder is from Snowflake Inc.
...
import ijson
import nltk
nltk.data.path.insert(0, os.path.abspath('../model/nltk'))
...
def prepare_schema_filter_data(question, db_info):
    ...
    for outer_table_idx, table_name_original in enumerate(db_info["table_names_original"]):
        ...
        for (inner_table_idx, column_name_original), (_, column_comment) in zip(
            db_info["column_names_original"], db_info["column_names"]
        ):
...
def obtain_db_details(
    db_info, data_source, sampled_db_values_dict, relavant_db_values_dict, output_seq, mode, question
):
    db_details = []
    ...
    for outer_table_idx, table_name in enumerate(db_info["table_names_original"]):
        ...
        for column_idx, ((inner_table_idx, column_name), (_, column_comment), column_type) in enumerate(
            zip(db_info["column_names_original"], db_info["column_names"], db_info["column_types"])
        ):
            if inner_table_idx == outer_table_idx:
                ...
                for source_column_idx, target_column_idx in db_info["foreign_keys"]:
                    if column_idx == source_column_idx:
                        ...
                        fk_info.append(
                            "    CONSTRAINT"
                            f' fk_{source_table_name.lower().replace(" ", "_")}_{source_column_name.lower().replace(" ", "_")} FOREIGN'
                            f" KEY ({format_identifier(source_column_name)}) REFERENCES"
                            f" {format_identifier(target_table_name)} ({format_identifier(target_column_name)}),"
                        )
        ...
        if len(column_info_list) > 0:
            ...
            if len(pk_columns) > 0:
                pk_info = [
                    "    PRIMARY KEY ("
                    + ", ".join([f"{format_identifier(column_name)}" for column_name in pk_columns])
                    + "),"
                ]
            else:
                pk_info = []
...
def prepare_input_output_pairs(
    data, ek_key, db_id2relevant_hits, sampled_db_values_dict, db_info, source, output_key, mode
):
    ...
    db_details = obtain_db_details(
        db_info, source, sampled_db_values_dict, relavant_db_values_dict, data[output_key], mode, question
    )

    input_seq = input_prompt_template.format(db_engine="SQLite", db_details=db_details, question=question)

    return {"question": question, "input_seq": input_seq, "output_seq": data[output_key], "db_desc": db_details, "db_id":data['db_id']}
...
if __name__ == "__main__":
    ...
    if os.path.exists(opt.output_data_file):
        print('--- column example 已存在，跳过！')

    else:
        print('--- column example 不存在，现在处理')
        random.seed(42)
        ...
        if opt.source == "synthetic":
            ...
            if opt.mode == "train":
                output_key = "SQL" #"cot"  # use our synthetic CoT during training
            else:
                output_key = "SQL"
            ek_key = "evidence"
        ...
        for db_info in tqdm(load_json_file(opt.tables)):
            ...
            db_file = os.path.join(opt.db_path, f"{opt.mode}", f"{opt.mode}_databases", db_id, db_id + ".sqlite")
