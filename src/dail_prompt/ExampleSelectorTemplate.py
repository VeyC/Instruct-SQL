...
import random
import os
from dail_utils.utils import sql2skeleton, jaccard_similarity
from dail_utils.linking_utils.application import mask_question_with_schema_linking
...
class CosineSimilarExampleSelector(BasicExampleSelector):
    def __init__(self, data, *args, **kwargs):
        ...
        local_model_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../model/sentence-transformers/all-mpnet-base-v2"))
        if os.path.isdir(local_model_path) and os.path.exists(os.path.join(local_model_path, "config.json")):
            print("having model to:", local_model_path)
            self.bert_model = SentenceTransformer(local_model_path, device="cpu")
        else:
            self.bert_model = SentenceTransformer("sentence-transformers/all-mpnet-base-v2", device="cpu")
            os.makedirs(local_model_path, exist_ok=True)
            self.bert_model.save(local_model_path)
...
class EuclideanDistanceExampleSelector(BasicExampleSelector):
    def __init__(self, data, *args, **kwargs):
        ...
        if os.path.isdir(local_model_path) and os.path.exists(os.path.join(local_model_path, "config.json")):
            ...
            self.bert_model = SentenceTransformer("sentence-transformers/all-mpnet-base-v2", device="cpu")
            os.makedirs(local_model_path, exist_ok=True)
            print("Saving model to:", local_model_path)
            self.bert_model.save(local_model_path)
        self.train_embeddings = self.bert_model.encode(self.train_questions)
